<script lang="ts">
    ...
    export let data;
    let spellbook;
    let spellsByLevel: Spell[][] = [];

    $: spellbook = $spellbookStore[data?.index];

    $: if (spellbook && spellbook.spells.length) {
        const groupedSpells: Spell[][] = [];
        for (let spell of spellbook.spells) {
            const level = spell.level_int;
            if (!groupedSpells[level]) {
                groupedSpells[level] = [];
            }
            groupedSpells[level].push(spell);
        }
        spellsByLevel = [...groupedSpells]; 
    }
</script>
...
<PrintablePage title={`${spellbook.name}'s Spellbook`}>
    <div class="space-y-4">
        ...
        {#if spellbook}
            {#each spellsByLevel as level}
                {#if level && level.length>0}
                    <p class="font-bold border-b-2 border-black w-full pb-1 text-2xl ">Level {level[0].level_int}</p>
                        {#each level as s}
