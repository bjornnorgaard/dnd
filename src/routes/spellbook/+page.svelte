<script lang="ts">
    ...
    function handleNameSort() {
        spells = [ ...originalSpellsOrder ].sort((a, b) => a.name.localeCompare(b.name));
    }
        ...
        <form on:submit|preventDefault={() => createSpellbook()} class="flex gap-4">
            <label for="input" class="w-full label">
                ...
                <span class="flex w-full gap-2">
                    ...
                    <button class="btn" class:variant-filled-surface={!ownerInput.length} class:variant-filled-primary={ownerInput.length} type="button" disabled={!ownerInput} on:click={() => createSpellbook()}>
                        Create
                    </button>
                </span>
            </label>
                    ...
                    {#if $activeSpellbookIndex !== 99}
                        <div class="mb-8">
                            ...
                            <div class="input-group input-group-divider grid-cols-[auto_1fr_auto]">
                                ...
                                <input class="input" type="search" placeholder="Search spells..." bind:value={searchInput} on:keyup={async (e) => await searchKeyUp(e)}/>
                            </div>
                            <div class="flex py-2 gap-2">
                                <DropDownLevels on:update={handleLevelUpdate}/>
                                            ...
                                            <AccordionItem>
                                                <svelte:fragment slot="summary">
                                                    <div class="flex justify-between gap-4">
                                                        ...
                                                        <div class="flex gap-2">
                                                            {#if s.can_be_cast_as_ritual}<span class="badge variant-outline-primary">Ritual</span>
                                                            {/if}
                                                ...
                                                <svelte:fragment slot="content">
                                                    <div class="px-2 pt-2 pb-4">
                                                        <div class="relative">
                                                            <button class="absolute top-0 right-0 btn btn-sm hover:variant-filled-error"
                                                                    on:click={() => removeSpell($activeSpellbookIndex, si)}>
                                                                Remove
                                                            </button>
                                                            <p>
                                                                <b>School</b>
                        ...
                        <div class="flex justify-between pt-8 gap-4">
                            ...
                            <button class="btn text-surface-600-300-token hover:variant-filled-error" type="reset" on:click={() => deleteSpellbook($activeSpellbookIndex)}>
